...
def update_movie():
    ...
    for movie in movies:
        if movie['title'] == title:
            movie['year'] = input(f"Year ({movie['year']}): ") or movie['year']
            movie['director'] = input(f"Director ({movie['director']}): ") or movie['director']
            movie['rating'] = float(input(f"Rating ({movie['rating']:.1f}): ") or movie['rating'])
            #  Add Notes in the Movie
...
def create_website():
    ...
    movies_html = ''
    for movie in movies:
        movies_html += f'<div class="movie"><img class="movie-poster" src="{movie["poster_url"]}">'
